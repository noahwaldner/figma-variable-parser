...
const buildTokens = async () => {
  ...
  let themeStyles = "";
  Promise.all([Bun.file(new URL("../.tmp/primitives.css", import.meta.url)).text().then(content => {
    themeStyles += content + "\n";
  }), ...config.themes.map(async (theme) => {
    return Bun.file(new URL(`../.tmp/${theme}.css`, import.meta.url)).text().then(content => {
      themeStyles += content + "\n";
    });
  })]).then(() => {
